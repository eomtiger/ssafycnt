...
function TrendItems(props) {
  ...
  if (Object.keys(props.anyItem[7]).length === 3) {
    labels = Object.keys(props.anyItem[7])
      .map((v) => {
        if (v !== "changeRate") {
          return v;
        }
    ...
    values = Object.values(props.anyItem[7])
      .map((v) => {
        if (typeof v != "object") {
          return v;
        }
      ...
      tooltip: {
        ...
        callbacks: {
          ...
          afterLabel: function (tooltipItem, data) {
            return (
              "증감율 : " +
              tooltipItem.dataset.data1[tooltipItem.dataIndex] +
              " %"
            );
          },
        },
      ...
      <div className="flex justify-between items-center ml-2 text-left mb-3 font-mun">
        <div>
          <div
            ...
            className="mt-1 font-bold text-base text-gray-12 w-40 truncate "
          >
            {pickItem}
          </div>
          <div
            id="tooltip-top"
            role="tooltip"
            className="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-sm opacity-0 tooltip dark:bg-gray-700 font-mun"
          >
            {pickItem}
            <div className="tooltip-arrow" data-popper-arrow></div>
          </div>
          <div className="mt-1 font-bold text-xl font-mun">
            {imOrExport} 추세
          </div>
        </div>
